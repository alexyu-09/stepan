...
### 1. Генерация SKU (Артикул), Алиасов и Описаний
Логика описана в `src/utils/generators.ts` и `src/utils/mapper.ts`.
...
- **AI Описания (RU ↔ UA)**: 
  - Автоматическая генерация уникальных коммерческих описаний на основе названия товара и бренда.
  - Используется **Google Gemini 2.0 Flash** для создания текстов (2-4 предложения).
  - **Batch Processing**: Товары обрабатываются пачками по 20 штук за один запрос к ИИ для высокой скорости и экономии лимитов.
  - **Caching**: Сгенерированные описания сохраняются в локальном кэше сессии.
  - Описание генерируется на русском языке и автоматически переводится на украинский.
- **Перевод (RU ↔ UA)**: Встроенная система перевода названий и описаний с кешированием (`translationCache`).
...
- **Выбор в один клик**: Нажатие на изображение мгновенно сохраняет его с правильным именем `{SKU}.jpg` и записывает в колонку «Фото» (индекс 16).
...
- `api/generate-description.ts`: Генерация описаний товаров через Google Gemini (Batch mode).
...
- `src/utils/mapper.ts`: Асинхронный маппинг, включая фазу батч-генерации описаний.
- `src/utils/generators.ts`: Генерация SKU/Alias, описаний и кэш переводов.
...
- `GEMINI_API_KEY`: Ключ API для генерации описаний через Google Gemini.
